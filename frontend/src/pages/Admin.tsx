...
import type { Producto, Mesa, Mesero, Categoria } from '../../../shared/index'
...
export default function Admin() {
  ...
  const { logout, dbId, apiFetch } = useAppwrite()
  ...
  const toggleMesero = async (mesero: Mesero) => {
    try {
      await databases.updateDocument(dbId, 'meseros', mesero.$id, {
        ...
        {activeTab === 'menu' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() => { setShowForm(true); setEditingItem(null); setFormProducto({ nombre: '', precio: '', categoria: '', descripcion: '' }) }}
                className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700"
              >
                + Agregar
              </button>
            ...
            {showForm && (
              <div className="bg-white rounded-xl border p-4 mb-4 space-y-3">
                ...
                />
                <div className="flex gap-2">
                  <button onClick={saveProducto} className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700">
                    {editingItem ? 'Actualizar' : 'Crear'}
                  </button>
                  <button onClick={() => setShowForm(false)} className="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-300">
                    Cancelar
                  </button>
                </div>
              </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${
                        p.disponible_hoy ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'
                      }`}
                    >
                      {p.disponible_hoy ? 'Disponible' : 'Agotado'}
                    </button>
                    <button
                      onClick={() => { setEditingItem(p); setShowForm(true); setFormProducto({ nombre: p.nombre, precio: String(p.precio), categoria: p.categoria, descripcion: p.descripcion || '' }) }}
        ...
        {activeTab === 'mesas' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() => { setShowForm(true); setEditingItem(null); setFormMesa({ numero: '', capacidad: '' }) }}
                className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700"
              >
                + Agregar
              </button>
            ...
            {showForm && (
              <div className="bg-white rounded-xl border p-4 mb-4 space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={saveMesa} className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold">
                    {editingItem ? 'Actualizar' : 'Crear'}
                  </button>
                  <button onClick={() => setShowForm(false)} className="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-semibold">
                    Cancelar
                  </button>
                </div>
              </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <span className={`inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-semibold ${
                        m.estado === 'libre' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                      }`}>
                        {m.estado}
                      </span>
                    </div>
                    <div className="flex gap-1">
                      <button
                        onClick={() => { setEditingItem(m); setShowForm(true); setFormMesa({ numero: String(m.numero), capacidad: String(m.capacidad) }) }}
                        className="p-1.5 rounded-lg bg-blue-100 text-blue-700 text-xs"
                      >
                        ✎
                      </button>
                      <button
                        onClick={() => deleteMesa(m.$id)}
                        className="p-1.5 rounded-lg bg-red-100 text-red-700 text-xs"
                      >
                        ✕
                      </button>
                    </div>
        ...
        {activeTab === 'meseros' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() => { setShowForm(true); setEditingItem(null); setFormMesero({ nombre: '', pin: '', telefono: '' }) }}
                className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700"
              >
                + Agregar
              </button>
            ...
            {showForm && (
              <div className="bg-white rounded-xl border p-4 mb-4 space-y-3">
                <input
                  ...
                  value={formMesero.nombre}
                  onChange={e => setFormMesero(f => ({ ...f, nombre: e.target.value }))}
                  className="w-full p-3 border rounded-lg text-sm"
                />
                <div className="grid grid-cols-2 gap-3">
                  <input
                    placeholder="PIN (4 digitos, auto si vacio)"
                    value={formMesero.pin}
                    onChange={e => setFormMesero(f => ({ ...f, pin: e.target.value }))}
                    className="p-3 border rounded-lg text-sm"
                    maxLength={4}
                  />
                  <input
                    placeholder="Telefono"
                    value={formMesero.telefono}
                    onChange={e => setFormMesero(f => ({ ...f, telefono: e.target.value }))}
                    className="p-3 border rounded-lg text-sm"
                  />
                </div>
                <div className="flex gap-2">
                  <button onClick={saveMesero} className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold">
                    {editingItem ? 'Actualizar' : 'Crear'}
                  </button>
                  <button onClick={() => setShowForm(false)} className="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-semibold">
                    Cancelar
            ...
            <div className="space-y-2">
              {meseros.map(m => (
                <div key={m.$id} className="flex items-center justify-between p-3 bg-white rounded-xl border shadow-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => toggleMesero(m)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${
                        m.activo ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'
                      }`}
                    >
                      {m.activo ? 'Activo' : 'Inactivo'}
                    </button>
                    <button
                      onClick={() => { setEditingItem(m); setShowForm(true); setFormMesero({ nombre: m.nombre, pin: m.pin, telefono: m.telefono || '' }) }}
                      className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-blue-100 text-blue-700"
                    >
                      Editar
                    </button>
...
// ── Ventas sub-component ──
function VentasTab({ apiFetch }: { apiFetch: (path: string, options?: RequestInit) => Promise<Response> }) {
  ...
  if (!stats) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-xs text-gray-400 mt-2">Asegurate de que el Worker este configurado con JWT_SECRET.</p>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <StatCard label="Ingresos" value={`$${(stats.ingresos_hoy || 0).toFixed(2)}`} color="blue" />
        <StatCard label="Propinas" value={`$${(stats.propinas_hoy || 0).toFixed(2)}`} color="green" />
        <StatCard label="Facturas" value={String(stats.facturas_hoy || 0)} color="purple" />
        <StatCard label="Ticket Prom." value={`$${(stats.ticket_promedio || 0).toFixed(2)}`} color="amber" />
      </div>

      <div className="grid grid-cols-2 gap-3">
        <StatCard label="Pedidos Activos" value={String(stats.pedidos_activos || 0)} color="amber" />
        <StatCard label="Mesas Ocupadas" value={`${stats.mesas_ocupadas || 0}/${stats.total_mesas || 0}`} color="red" />
      </div>
    </div>
  )
}
